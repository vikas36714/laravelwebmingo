...
<body>
				...
				<div class="myform form ">
					...
					<form method="POST" action="{{ route('login') }}" name="login">
						...
						<div class="form-group">
                            ...
                            <x-jet-input id="email" class="form-control" type="email" name="email" :value="old('email')" autofocus placeholder="Enter Email" />
						</div>
						<div class="form-group">

                            <x-jet-label for="password" value="{{ __('Password') }}" />
                            <x-jet-input id="password" class="form-control" type="password" name="password" autocomplete="current-password" placeholder="Enter Password"/>
						</div>
						...
						<div class="form-group text-center">
                            {{-- @if (Route::has('password.request'))
                                <a class="underline text-sm text-gray-600 hover:text-gray-900" href="#forgot-pass" data-target="#forgot-pass" data-toggle="modal">
                                    {{ __('Forgot password?') }}
                                </a>
                            @endif --}}
                            <a class="underline text-sm text-gray-600 hover:text-gray-900" href="#forgot-pass" data-target="#forgot-pass" data-toggle="modal">
                ...
                <span class="success" style="color:green; margin-top:10px; margin-bottom: 10px;"></span>
					<div class="form-group row">
						<div class="col-sm-12">
                            ...
                            <x-jet-input class="form-control" id="email_id" type="email" name="email" placeholder="Enter Registered Email Address" />
						</div>
					</div>
					<div class="form-group row">
    ...
    <script>
        ...
        $('body').on('click', '#resetbtn', function (event) {
            ...
            $.ajax({
            url: <?php echo json_encode(route('forget-password')) ?>,
            type: "POST",
            data: {
                "_token": "{{ csrf_token() }}",
                email: email_id
